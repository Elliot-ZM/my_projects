...
class Combined_Dataset(Dataset):
    def __init__(self, args, mode, transform = False):
        ...
        if self.mode == 'train':
            imgs = glob.glob(os.path.join(self.root, 'train') + '**/**/*.jpg', recursive = True)
            # diode (outdoor) + rs
            ...
            imgs = glob.glob(os.path.join(self.root, 'val') + '**/**/*.jpg', recursive = True)
            # diode (outdoor) + rs
    ...
    def __getitem__(self, index):
        ...
        elif not 'outdoors' in self.imgs[index]:
            self.dataset = 'realsense'
            rgb_path = self.imgs[index]
            depth_path = rgb_path[:-4] + '_depth.npy'
            mask_path = None
        
        elif 'outdoors' in self.imgs[index]:
            self.dataset = 'diode'
            rgb_path = self.imgs[index]
            depth_path = rgb_path[:-4] + '_depth.npy'
        ...
        if mode == 'train':
            ...
            if debug:
                ...
                for i in range(len(sample['image'])):
                    img = to_numpy(sample['image'][i])
                    depth = to_numpy(sample['depth'][i], cmap= 'jet')
...
if __name__ == '__main__':
    ...
    ap.add_argument('--data_root', type=str, help= 'dataset directory', default= '/mnt/520ED4640ED44321/ZMH/datasets/depth_dataset/nyu+diode+rs')
    ...
    # test_loader = Combined_Dataloader(args, mode = 'test')
